        ...
        if ($this->table->hasColumns()) {
            ...
            if (!empty($relationColumns)) {
                ...
                $groupColumns = array_filter(
                    ...
                    fn ($column) => $column instanceof PropertyColumn && !$column->hasRelation()
                );
                ...
                $groupColumns = array_map(function ($column) {
                    if (preg_match('/^".+".* as (.+)$/i', $column, $matches)) {
                        return $matches[1];
                    }
                    return $column;
                }, $groupColumns);

                $query->groupByRaw(implode(', ', $groupColumns));
            }
